...
def depth_limited_search(problem, limit=10):
    ...
    node = Node(problem.initial)
    return recursive_DSL(node,problem,limit)

def recursive_DSL(node,problem,limit,occur = False):
    if problem.goal_test(node.state):
        return node
    elif limit == 0:
        return "cutoff"
        ...
        for child in node.expand(problem):
            result = recursive_DSL(child,problem,limit - 1,occur)
            if result == "cutoff":
                occur = True
            elif result != "fail":
                return result
...
def iterative_deepening_search(problem):
    ...
    depth = 0;
    result = depth_limited_search(problem,30)
    while(result == "cutoff"):
...
if __name__ == '__main__':
    ...
    #result3 = depth_limited_search(problem)
