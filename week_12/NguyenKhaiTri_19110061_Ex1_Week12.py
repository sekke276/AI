...
def min_conflicts(csp, max_steps=100000):
    ...
    for var in csp.variables:
        val = min_conflicts_value(csp, var, current)    # Choose the least confict value
        csp.assign(var, val, current)   # put the least confict value in to assignment
    
    for i in range(max_steps):  
        conflicted = csp.conflicted_vars(current)   # assign list conflisted   
        if not conflicted:      #check conflicted
            return current      #end the solution and return the solution if find out the solution
        var = random.choice(conflicted) # choose the random value that in the conflicted list
        val = min_conflicts_value(csp, var, current) # choose the minimum value in conflicted list
        csp.assign(var, val, current)       # assign value in list conflicted
